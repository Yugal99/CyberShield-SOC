...
const ACCEPTED_EXTENSIONS = ['txt', 'json', 'csv', 'log'];
const SAMPLE_LOGS = [
  { timestamp: 'Jun 14 02:11:43', ip: '203.0.113.4', username: 'root', event: 'Failed Password', status: 'FAIL' },
  { timestamp: 'Jun 14 02:11:45', ip: '203.0.113.4', username: 'root', event: 'Failed Password', status: 'FAIL' },
  { timestamp: 'Jun 14 02:11:47', ip: '203.0.113.4', username: 'root', event: 'Failed Password', status: 'FAIL' },
  { timestamp: 'Jun 14 02:11:49', ip: '203.0.113.4', username: 'root', event: 'Failed Password', status: 'FAIL' },
  { timestamp: 'Jun 14 02:11:51', ip: '203.0.113.4', username: 'root', event: 'Failed Password', status: 'FAIL' },
  { timestamp: 'Jun 14 02:12:01', ip: '203.0.113.4', username: 'admin', event: 'Invalid User', status: 'FAIL' },
  { timestamp: 'Jun 14 02:15:11', ip: '', username: '', event: 'Sudo Authentication Failure', status: 'FAIL' },
  { timestamp: 'Jun 14 02:15:14', ip: '', username: '', event: 'Sudo Authentication Failure', status: 'FAIL' },
  { timestamp: 'Jun 14 02:18:25', ip: '10.0.0.5', username: 'jdoe', event: 'Login Success', status: 'SUCCESS' },
];
...
const normalizeLog = (item) => ({
  ...
  event: item.event || item.event_type || item.type || 'Imported Event',
  status: String(item.status || item.result || 'INFO').toUpperCase(),
});
...
function parseDelimited(text) {
  ...
  return rows.slice(1).map((row) => {
    ...
    return normalizeLog(item);
  });
}

function parsePlainLogs(text) {
  return text.split(/\r?\n/).filter(Boolean).map((line) => {
    const timestamp = line.match(/^([A-Z][a-z]{2}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2})/)?.[1] || '';
    const ip = line.match(/(\d{1,3}(?:\.\d{1,3}){3})/)?.[1] || '';
    const failed = line.match(/Failed password for (?:invalid user )?(\w+)/);
    const invalid = line.match(/Invalid user (\w+)/);
    const success = line.match(/Accepted password for (\w+)/);
    if (failed) return { timestamp, ip, username: failed[1], event: 'Failed Password', status: 'FAIL' };
    if (invalid) return { timestamp, ip, username: invalid[1], event: 'Invalid User', status: 'FAIL' };
    if (success) return { timestamp, ip, username: success[1], event: 'Login Success', status: 'SUCCESS' };
    if (line.toLowerCase().includes('sudo') && line.toLowerCase().includes('failure')) return { timestamp, ip, username: '', event: 'Sudo Authentication Failure', status: 'FAIL' };
    return { timestamp, ip, username: '', event: 'Log Entry', status: 'INFO' };
  });
}

function parseFileContent(file, text) {
  const ext = extensionOf(file.name);
  if (ext === 'json') {
    const parsed = JSON.parse(text);
    return (Array.isArray(parsed) ? parsed : [parsed]).map(normalizeLog);
  }
  if (ext === 'csv') return parseDelimited(text);
  return parsePlainLogs(text);
}
...
function App() {
  ...
  const addFiles = async (fileList) => {
    ...
    for (const file of supported) {
      ...
      const parsed = parseFileContent(file, text);
    ...
    if (fileSummaries.length) {
      ...
      setMessage('Unsupported file type. Please upload .txt, .json, .csv, or .log files.');
    }
  };

  const stats = useMemo(() => {
    const failures = logs.filter((log) => log.status === 'FAIL').length;
    ...
    <section className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
      <header className="overflow-hidden rounded-[2rem] bg-[#1C1C1C] text-white shadow-2xl">
        <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1.2fr_.8fr] lg:p-10">
          <div>
            <p className="mb-3 inline-flex rounded-full bg-[#FE5729] px-4 py-2 text-sm font-semibold">CyberShield SOC Frontend</p>
            ...
            <p className="mt-4 max-w-2xl text-base text-white/70 sm:text-lg">A responsive Vite + React + Tailwind dashboard for SOC analysts to ingest files, inspect parsed output, and triage suspicious authentication activity.</p>
          </div>
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_.85fr]">
        <div className="rounded-[1.5rem] bg-[#1C1C1C] p-4 text-white shadow-xl sm:p-6">
          ...
          >
            ...
            <button className="mt-5 rounded-full bg-[#FE5729] px-6 py-3 font-bold text-white shadow-lg shadow-[#FE5729]/30" type="button">Select .txt, .json, .csv, or .log</button>
            <input ref={inputRef} className="hidden" type="file" multiple accept=".txt,.json,.csv,.log" onChange={(event) => addFiles(event.target.files)} />
          </div>
        </div>
        ...
        <div className="rounded-[1.5rem] bg-[#1C1C1C] p-5 text-white shadow-xl">
          ...
          <div className="mx-auto mt-6 grid max-w-xs place-items-center">
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-3 text-xs text-white/70">
              ...
              <span><b className="text-[#D4B5FF]">●</b> Info</span>
            </div>
          </div>
        ...
        <div className="rounded-[1.5rem] bg-white p-4 shadow-xl sm:p-6">
          ...
          <div className="mb-4 grid gap-3 sm:grid-cols-[1fr_auto]">
            ...
            <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="rounded-2xl border border-black/10 bg-[#F1F1F1] px-4 py-3 font-semibold outline-none focus:border-[#FE5729]">
              {['ALL', 'FAIL', 'SUCCESS', 'INFO'].map((status) => <option key={status}>{status}</option>)}
            </select>
          </div>
